from hosaka.schemas import SpeechParams, SpeechRequest, clamp_params
...
def test_clamp_bounds_values():
    p = SpeechParams(exaggeration=5.0, cfg_weight=-1.0, temperature=9.0, speed=0.01)
    c = clamp_params(p)
...
def test_clamp_leaves_valid_values():
    p = SpeechParams(exaggeration=0.6, cfg_weight=0.4, temperature=0.8, speed=1.1)
