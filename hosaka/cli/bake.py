...
from hosaka.config import BAKE_SEED_TEXT, SAMPLE_RATE, VOICE_DIR
...
def bake(description: str, voice_id: str, text: str = BAKE_SEED_TEXT) -> str:
    ...
    VoiceLibrary(VOICE_DIR).add(
        voice_id, out_path, source="bake", params={"description": description}
    )
    return str(out_path)
...
def main():
    ...
    ap.add_argument("--text", default=BAKE_SEED_TEXT, help="sentence to speak for the seed clip")
    args = ap.parse_args()
