...
def main(tmp="/tmp/hosaka_bench") -> int:
    ...
    k_times = [first_chunk_ms(kok.stream(SHORT, "af_heart", {"speed": 1.0}))
               for _ in range(RUNS)]
    k_median = statistics.median(k_times)
    print(f"kokoro (realtime path): first-chunk ms "
    ...
    full_gen_ms(cb, SHORT, "bench_seed",
                {"exaggeration": 0.5, "cfg_weight": 0.4, "temperature": 0.8})  # warm
    cb_runs = [full_gen_ms(cb, SHORT, "bench_seed",
                           {"exaggeration": 0.5, "cfg_weight": 0.4,
                            "temperature": 0.8}) for _ in range(RUNS)]
    cb_times = [t for t, _dur in cb_runs]
    print(f"chatterbox (quality mode): full-generate ms "
    ...
    if k_median > 1000.0:
        ...
        return 1
    print("GATE PASS: Kokoro realtime path within budget. "
